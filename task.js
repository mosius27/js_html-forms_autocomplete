class Autocomplete {
  ...
  getMatches( text ) {
    ...
    */
    ...
    const options = this.input.querySelectorAll('option'); // Получаем список всех опций

    const matches = [];

    options.forEach(option => {
      const optionText = option.textContent.toLowerCase(); // Преобразуем текст опции в нижний регистр
      if (optionText.includes(text.toLowerCase())) { // Проверяем, содержит ли текст опции введенный текст
        matches.push({
          text: optionText,
          value: option.value
        });
